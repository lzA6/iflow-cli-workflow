...
class ScriptHealthChecker:
    ...
    def check_all_scripts(self) -> Dict:
        """检查所有Python脚本"""
        ...
        for py_file in python_files:
            ...
            success, error = self._check_script_import(script_name, py_file)
    ...
    def _check_script_import(self, module_name: str, file_path: Path) -> Tuple[bool, str]:
        """检查单个脚本导入"""
        try:
            # 动态导入模块
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("=" * 60)
        report.append("Python Script Health Check Report")
        report.append("=" * 60)
...
def main():
    ...
    print("Starting script health check...")
    print(f"Check directory: {current_dir}")
    print("-" * 60)
    
    # 创建检查器
    checker = ScriptHealthChecker(current_dir)
    ...
    # 返回退出码
    sys.exit(0 if results['failed_imports'] == 0 else 1)
