---
description: 基于可用的设计工件为功能生成可操作的、依赖有序的 tasks.md。
---
...
1. **设置**：从仓库根目录运行 `.specify/scripts/bash/check-prerequisites.sh --json` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于参数中的单引号，如 "I'm Groot"，请使用转义语法：例如 'I'\''m Groot'（或者如果可能的话使用双引号："I'm Groot"）。
...
3. **执行任务生成工作流**（遵循模板结构）：
   ...
   - **按用户故事组织生成任务**：
     ...
     - 对于每个用户故事（按优先级顺序 P1、P2、P3...）：
       ...
       - 如果请求测试：包括该故事特定的测试
     - 抛光/集成任务（跨领域关注点）
   - **测试是可选的**：仅在功能规范中明确请求测试或用户要求 TDD 方法时生成测试任务
   - 应用任务规则：
     - 不同文件 = 标记 [P] 为并行
     - 相同文件 = 顺序（无 [P]）
...
**重要提示**：测试是可选的。仅当功能规范中明确请求测试或用户要求 TDD 方法时才生成测试任务。
...
2. **来自合约**：
   ...
   - 如果请求测试：每个合约 → 在该故事阶段的实施之前进行合约测试任务 `[P]`
   
3. **来自数据模型**：
   - 将每个实体 → 映射到需要它的用户故事
   - 如果实体服务于多个故事：将其放在最早的故事或设置阶段
   - 关系 → 适当故事阶段的服务层任务
...
5. **排序**：
   - 阶段 1：设置（项目初始化）
   - 阶段 2：基础（阻塞性先决条件 - 必须在用户故事之前完成）
   - 阶段 3+：按优先级顺序的用户故事（P1、P2、P3...）
     - 在每个故事中：测试（如果请求）→ 模型 → 服务 → 端点 → 集成
   - 最后阶段：完善和跨领域关注
   - 每个用户故事阶段都应该是一个完整的、可独立测试的增量
